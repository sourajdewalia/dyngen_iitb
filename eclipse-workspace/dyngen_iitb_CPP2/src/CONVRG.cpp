  ...
  int& icon)
{
  ...
  statement_101:
  //C
  procom(cmnprocom, far, ti, xx1, xx2, xx3, xx4, phii, xx6);
  //C
  ...
  j++;
  //C
  procom(cmnprocom, far, tss, css, aks, cp, rexs, phiss, hss);
  ...
  statement_6:
  j++;
  thermo(cmnthrmo, pa, hsi, tsi, ssi, xxi, 1, far, 0);
  ...
  tso = tsi;
  pso = pa;
  procom(cmnprocom, far, tso, cso, xx2, xx3, rex, phiso, hso);
  ...
  statement_15:
  j++;
  procom(cmnprocom, far, tso, cso, ako, cp, rex, phiso, hso);
  rho = capsf * pso / (aj * rex * tso);
  vo = wg / (rho * ao);
