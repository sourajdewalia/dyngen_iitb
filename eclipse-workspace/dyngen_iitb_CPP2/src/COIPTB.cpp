...
struct common_hturb
{
  ...
  common_hturb() :
    tffy(dimension(15), fem::fill0),
    cny(dimension(15, 15), fem::fill0),
    dhtcy(dimension(15, 15), fem::fill0),
    etaty(dimension(15, 15), fem::fill0),
    ntfys(fem::int0),
    npttsf(dimension(15), fem::fill0)
  {}
};
  ...
  common& cmn)
{
  ...
  float& cnip = cmn.cnip;
  float& etatip = cmn.etatip;
  float& dhtcip = cmn.dhtcip;
  float& dhti = cmn.dhti;
  float& blip = cmn.blip;
  float& tfipcf = cmn.tfipcf;
  float& cnipcf = cmn.cnipcf;
  float& etipcf = cmn.etipcf;
  float& wg50 = cmn.wg50;
  float& far50 = cmn.far50;
  bool& fxm2cp = cmn.fxm2cp;
  float& viptrb = cmn.viptrb;
  float& waip = cmn.waip;
  bool& si = cmn.si;
  ...
  arr_cref<float> tffy(cmn.tffy, dimension(15));
  arr_cref<float, 2> cny(cmn.cny, dimension(15, 15));
  arr_cref<float, 2> dhtcy(cmn.dhtcy, dimension(15, 15));
  arr_cref<float, 2> etaty(cmn.etaty, dimension(15, 15));
  arr_cref<int> npttsf(cmn.npttsf, dimension(15));
  //
  ...
  cnipcf = cmn.cnipds * fem::sqrt(t50) / pcni;
  if (fxm2cp) {
  ...
  statement_2:
  if (fxm2cp) {
    search(-1.f, tffip, cnip, dhtcip, etatip, tffy(1), cmn.ntfys, cny(1,
  ...
  xnp = cmn.xnipds * pcni / 100.f;
  xnidot = deriv(cmndrv, 2, xnip);
  btuext = 0.706705f * hpext;
  if (si) {
    btuext = hpext;
  }
  dhacel = confac * cmn.pmiip * xnip * xnidot;
  dhtic = (waip * (h21 - h22) * dhacel) / (wg50 * t50);
  if (fxm2cp) {
    cmn.dhtc = (btuext + cmn.wacp * (h3 - h21) + waip * (h21 - h22) +
      dhacel) / (mg60 * t50);
  }
  if (ides == 0) {
    goto statement_6;
  }
  tfpcf = cmn.tfipds / tfical;
  dhpcf = ohtic / dhtcip;
  etipcf = cmn.etipds / etatip;
  if (fxm2cp) {
  ...
  thturb(dhti, etatip, far50, cmn.h50, cmn.s50, p50, t5, h5, s5, p5);
  if (blip <= 0.0f) {
    goto statement_7;
  }
  ...
  thermo(p5, h5, t5, s5, xx2, 1, far5, 1.f);
  goto statement_8;
  statement_7:
  far5 = far50;
  wg5 = wg50;
  ...
  wg5p = wg5;
  h5p = h5;
  psdot = deriv(cmndrv, 14, p5);
  statement_18:
  thermo(p5, h5, t5, s5, xx2, 1, far5, 0);
  wg5 = wg5p - psdot * viptrb / t5 / 1.4f / ra;
  u5 = h5 - ra * aj * t5;
  u5dot = deriv(cmndrv, 15, u5);
